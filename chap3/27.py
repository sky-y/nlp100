...
for k, v in dict_result.items():
    ...
    pattern = r"""
        ...
        [']{2,4}    # 「''」「'''」「''''」にマッチ
    """
    str_replaced = re.sub(pattern, r"\1", v, flags=re.VERBOSE)

    # 内部リンクマークアップを除去
    # ただし「:」を含む場合は除去しない（「ファイル:hoge」の場合など）
    pattern = r"""
        \[\[                # 「[[」にマッチ
        ((?!ファイル:).+?)    # 任意の文字列（1文字以上）にマッチ（非貪欲）、ただし「ファイル:」から開始しない
        \]\]                # 「]]」にマッチ
    """
    str_replaced = re.sub(pattern, r"\1", str_replaced, flags=re.VERBOSE)
